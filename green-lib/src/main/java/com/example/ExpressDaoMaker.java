...
public class ExpressDaoMaker {
    public static void main(String[] args) {
        ...
        try {
            //指定路径
            new DaoGenerator().generateAll(schema, "C:\\XQS\\yunsu-pda\\yang-express\\src\\main\\java-gen");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void addPackAndProduct(Schema schema) {
        ...
        Entity express =schema.addEntity("Express");
        express.addIdProperty().autoincrement();
        express.addStringProperty("packKey").index();
        express.addStringProperty("expressKey").index();
        express.addStringProperty("createTime").index();
